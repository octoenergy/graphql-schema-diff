...
from schemadiff.changes.field import FieldArgumentAdded, FieldDescriptionChanged
...
class ValidationRule(ABC):
    ...
    @classmethod
    def get_subclasses_by_names(cls, names: list[str]) -> set[type["ValidationRule"]]:
        if not names:
            return set()
    ...
    def is_valid(self) -> bool:
        ...
        if isinstance(added_type, GraphQLObjectType):
            all_its_fields_have_description = all(
                field.description not in EMPTY for field in added_type.fields.values()
            )
...
class RemoveTypeDescription(ValidationRule):
    ...
    @property
    def message(self) -> str:
        return f"Description for type `{self.change.type}` was removed (rule: `{self.name}`)"
