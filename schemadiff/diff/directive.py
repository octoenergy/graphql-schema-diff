...
class Directive:
    def __init__(self, old_directive, new_directive) -> None:
    ...
    def diff(self):
        ...
        changes.extend(
            DirectiveArgumentAdded(
                self.new_directive, argument_name, self.new_directive.args[argument_name]
            )
            for argument_name in added
        )
        changes.extend(
            DirectiveArgumentRemoved(
                self.new_directive, argument_name, self.old_directive.args[argument_name]
            )
...
class DirectiveArgument:
    def __init__(self, directive, arg_name, old_arg, new_arg) -> None:
        ...
        if self.old_arg.default_value != self.new_arg.default_value:
            changes.append(
                DirectiveArgumentDefaultChanged(
                    self.directive,
                    self.arg_name,
                    self.old_arg.default_value,
                    self.new_arg.default_value,
                )
            )
        if self.old_arg.description != self.new_arg.description:
            changes.append(
                DirectiveArgumentDescriptionChanged(
                    self.directive,
                    self.arg_name,
                    self.old_arg.description,
                    self.new_arg.description,
                )
            )
