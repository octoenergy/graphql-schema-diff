...
from schemadiff.__main__ import cli, main, parse_args
...
def test_schema_default_mode(capsys):
    ...
    assert (
        "⚠️ Default value for argument `x` on field `Field.calculus` changed from `0` to `100`"
        in stdout.out
    )
...
def test_schema_default_mode_json_output(capsys):
    ...
    ANOTHER_SCHEMA_FILE = "tests/data/simple_schema_dangerous_changes.gql"
    args = parse_args(
        ["--old-schema", SCHEMA_FILE, "--new-schema", ANOTHER_SCHEMA_FILE, "--as-json"]
    )
    exit_code = main(args)
    assert exit_code == 0
...
def test_cli_with_allow_list_does_not_show_allowed_changes(capsys):
    ...
    args = parse_args(
        ["--old-schema", SCHEMA_FILE, "--new-schema", ANOTHER_SCHEMA_FILE, "-a", ALLOW_LIST]
    )
    exit_code = main(args)
    assert exit_code == 0
...
def test_schema_strict_mode(capsys):
    SCHEMA_FILE = "tests/data/simple_schema.gql"
    ANOTHER_SCHEMA_FILE = "tests/data/simple_schema_dangerous_changes.gql"
    args = parse_args(["-o", SCHEMA_FILE, "--new-schema", ANOTHER_SCHEMA_FILE, "--strict"])
    exit_code = main(args)
...
def test_schema_rules_mode(capsys):
    ...
    args = parse_args(
        ["-o", SCHEMA_FILE, "--new-schema", ANOTHER_SCHEMA_FILE, "--validation-rules", *RULES]
    )
    exit_code = main(args)
    ...
    assert (
        "⛔ Type `NewTypeWithoutDesc` was added without a description for NewTypeWithoutDesc "
        "(rule: `add-type-without-description`)" in stdout.out
    )
    assert (
        "⛔ Type `NewEnumWithoutDesc` was added without a description for NewEnumWithoutDesc "
        "(rule: `add-type-without-description`)" in stdout.out
    )
    assert (
        "⛔ Description for type `Field` was removed "
        "(rule: `remove-type-description`)" in stdout.out
    )
    assert (
        "⛔ `Field.calculus` description was removed "
        "(rule: `remove-field-description`)" in stdout.out
    )
    assert (
        "⛔ Field `c` was added to object type `Query` without a description for "
        "Query.c (rule: `add-field-without-description`)" in stdout.out
    )
    assert (
        "⛔ Enum value `VALUE_3` was added to `Enum` enum without a description "
        "(rule: `add-enum-value-without-description`)" in stdout.out
    )
    assert (
        "⛔ Description for enum value `VALUE_2` was removed "
...
def test_schema_tolerant_mode(capsys):
    SCHEMA_FILE = "tests/data/simple_schema.gql"
    ANOTHER_SCHEMA_FILE = "tests/data/simple_schema_dangerous_changes.gql"
    args = parse_args(["-o", SCHEMA_FILE, "--new-schema", ANOTHER_SCHEMA_FILE, "--tolerant"])
    exit_code = main(args)
...
def test_tolerant_mode_doesnt_allow_breaking_changes(capsys):
    SCHEMA_FILE = "tests/data/simple_schema.gql"
    BREAKING_CHANGES_SCHEMA = "tests/data/simple_schema_breaking_changes.gql"
    tolerant_args = parse_args(["-o", SCHEMA_FILE, "-n", BREAKING_CHANGES_SCHEMA, "--tolerant"])
