...
from schemadiff.allow_list import InvalidAllowlist, read_allowed_changes
...
def test_allow_list_keys_are_not_change_checksums():
    with pytest.raises(InvalidAllowlist, match="All keys must be a valid md5 checksum"):
        read_allowed_changes(
            json.dumps(
                {"1e3b776bda2dd8b11804e7341bb8b2d1": "md5 checksum key", "bad key": "some message"}
            )
        )
